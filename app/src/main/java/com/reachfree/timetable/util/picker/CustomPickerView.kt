package com.reachfree.timetable.util.picker
...
class CustomPickerView : ScrollView {
    ...
    fun setItems(list: List<Any>) {
        ...
        for (i in 0 until offset) {
            items!!.add(0, Semester(null))
            items!!.add(Semester(null))
        }
        initData()
    }
        ...
        scrollerTask = Runnable {
            ...
            if (initialY - newY == 0) { // stopped
                try {
                    ...
                    Timber.d("ERROR: NumberFormatException $e")
                }
            } else {
                initialY = scrollY
    ...
    private fun onSelectedCallBack() {
        onCustomPickerViewListener.onSelected(selectedIndex, items!![selectedIndex])
    }

    fun setSelection(position: Int) {
        selectedIndex = position + offset
        this.post { this.smoothScrollTo(0, position * itemHeight) }
    }
