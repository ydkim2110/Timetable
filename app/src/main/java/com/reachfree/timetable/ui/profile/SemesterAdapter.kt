...
class SemesterAdapter(
    private val semesters: List<SemesterTotalCreditResponse>
) : RecyclerView.Adapter<SemesterAdapter.MyViewHolder>() {

    inner class MyViewHolder(
        private val binding: ItemSemesterBinding
    ) : RecyclerView.ViewHolder(binding.root) {
        fun bind(semester: SemesterTotalCreditResponse) {
            with(binding) {
    ...
    private var onItemClickListener: ((SemesterTotalCreditResponse) -> Unit)? = null

    fun setOnItemClickListener(listener: (SemesterTotalCreditResponse) -> Unit) {
